...
### Jawab:
1. relationship() vs options(): 
- relationship() digunakan untuk melakukan query secara dinamis ke database berdasarkan relasi Eloquent
- options() digunakan untuk menyuplai daftar opsi secara statis dari array.
...
4. HasMany vs BelongsTo: 
- HasMany digunakan di model Parent (yang tabel databasenya tidak meletakkan foreign key), contohnya Kategori memiliki banyak Post. 
- BelongsTo digunakan di model Child (yang tabel databasenya menampung foreign key category_id), contohnya Post milik sebuah Kategori.